...
@section('content')
    ...
    <div class="card card-block sameheight-item">
        <div class="title-block">
            <h3 class="title">
                {{ Lang::get('preylang.setting.userGroup.editUserGroupTitle') }}
            </h3>
        </div>
        ...
        <form role="form" method="POST" action="{{ route('userGroups.update', $userGroup->id) }}">
            @csrf
            @method('PUT')
            <div class="form-group row {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control boxed {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" name="name" value="{{ $errors->has('name') ? old('name') : $userGroup->name }}" autofocus>
                </div>
            </div>
            ...
            <div class="form-group row {{ $errors->has('name_kh') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control boxed {{ $errors->has('name_kh') ? 'is-invalid' : '' }}" id="name_kh" name="name_kh" value="{{ $errors->has('name_kh') ? old('name_kh') : $userGroup->name_kh }}" autofocus>
                </div>
            </div>
