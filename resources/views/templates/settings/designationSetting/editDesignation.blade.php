...
@section('content')
    ...
    <div class="card card-block sameheight-item">
        <div class="title-block">
            <h3 class="title">
                {{ Lang::get('preylang.setting.edit', ['setting' => Lang::get('preylang.setting.designation')]) }}
            </h3>
        </div>
        ...
        <form role="form" method="POST" action="{{ route('designation.update', $designation->id) }}">
            @csrf
            @method('PUT')
            ...
            <div class="form-group row {{ $errors->has('name') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control boxed {{ $errors->has('name') ? 'is-invalid' : '' }}" id="name" name="name" value="{{ $errors->has('name') ? old('name') : $designation->name }}" autofocus>
                </div>
            </div>
